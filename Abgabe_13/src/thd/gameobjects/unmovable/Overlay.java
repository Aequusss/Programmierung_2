...
public class Overlay extends GameObject {
     ...
     * @param fontName        Name of the font to be used for the message.
     */
    public Overlay(GameView gameView, GamePlayManager gamePlayManager, String fontName) {
    ...
    public void updateStatus() {
        if (messageShown && gameView.timer(millisecondsToShow, 0, this)) {
            messageShown = false;
        }
    }
    ...
    public void addToCanvas() {
        if (messageShown) {
            gameView.addTextToCanvas(message, position.getX(), position.getY(), size, false, Color.WHITE, 0, fontName);
        }
    }
}