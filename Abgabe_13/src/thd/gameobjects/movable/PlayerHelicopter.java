...
public class PlayerHelicopter extends CollidingGameObject implements MainCharacter {
    ...
    public void left() {
        if (position.getX() > LEFT_BOUNDARY) {
            ...
            // Only allow backwards world scrolling if we haven't rolled all the way back to the start frame
            if (!gamePlayManager.isAtStartOfLevel()) {
    ...
    public void addToCanvas() {
        ...
        if (isShielded) {
            int shieldWidth = 90;  // Slightly larger than the helicopter bounds
            int shieldHeight = 60;

            double manualOffsetX = 15.0; // Increase to move RIGHT, decrease to move LEFT
            double manualOffsetY = 15.0; // Increase to move DOWN, decrease to move UP
