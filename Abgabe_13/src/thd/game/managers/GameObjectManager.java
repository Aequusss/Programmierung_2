...
class GameObjectManager extends CollisionManager {
    ...
    void processFrame() {
        ...
        for (GameObject gameObject : gameObjects) {
            if (!(gameObject instanceof PlayerHelicopter || gameObject instanceof Background)) {

                if (gameObject.getPosition().getX() < -250) {
                    remove(gameObject);
                    continue;
                }
                if (gameObject.getPosition().getX() > GameView.WIDTH + 250) {
                    continue;
                }
            }
    ...
    /**
     * Provides access to the current list of active game objects.
     *
