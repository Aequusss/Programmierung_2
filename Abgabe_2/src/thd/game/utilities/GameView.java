...
public final class GameView {
     ...
     * }
     * }</pre>
     * Mit {@link MouseEvent#getButton()} lässt sich ermitteln, welche Maustaste betätigt wurde (links, rechts oder die
     * Mitte).
     *
    ...
    /**
     * Mit dieser Methode kann ein Sound (eine .wav-Datei) vorab geladen werden, um die Ladezeit beim Abspielen zu
     * reduzieren. Diese Methode sollte z.B. beim Wechsel des Levels aufgerufen werden, um Sounds für das nächste Level
     * vorab zu laden.
     * <p>
     * Die Sound-Datei muss in einem Verzeichnis "src/resources" liegen. Bitte den Namen der Datei ohne Verzeichnisnamen
     * angeben, z.B. <code>preloadSound("boom.wav")</code>. Der Dateiname darf nur aus Kleinbuchstaben bestehen.
     *
     * @param soundFile Name der Sound-Datei. Die Sound-Datei muss in einem Verzeichnis "src/resources" liegen und auf
     *                  ".wav" enden. Die Datei wird nur einmal geladen, auch wenn diese Methode mehrfach aufgerufen
     *                  wird.
     */
     ...
     * <p>
     * Beim erstmaligen Aufruf dieser Methode wird der Sound in den Speicher geladen, was einige Zeit in Anspruch nehmen
     * kann. Die Ladezeit kann durch die Methode {@link #preloadSound(String)} reduziert werden, die den Sound vorab
     * lädt.
     *
     * @param soundFile Name der Sound-Datei. Die Sound-Datei muss in einem Verzeichnis "src/resources" liegen und auf
     *                  ".wav" enden.
     * @param replay    Legt fest, ob der Sound endlos wiederholt werden soll.
     * @param volume    Die Lautstärke des Sounds. 0.0 = lautlos, 1.0 = normale Lautstärke, 2.0 = doppelte Lautstärke.
     ...
     */
    public int playSound(String soundFile, boolean replay, float volume) {
        return this.sound.playSound(soundFile, replay, volume);
    }
    ...
    private static class ClipPool {
        ...
        private Clip createClip() {
            try {
                ...
                if (globalClipCount > 32) {
                    throw new SoundException("Zu viele offene Sound-Dateien: " + globalClipCount
                            + ". Bitte entfernen Sie nicht mehr benötigte Sound-Dateien mit unloadSound(String soundFile) aus dem Speicher.");
                } else {
                    throw new SoundException(
                            "Fehler beim Erstellen des Clips für die Sound-Datei \"" + soundFile + "\": "
                                    + e.getMessage(), e);
                }
    ...
    private static class Sound {
        ...
        private int playSound(String soundFile, boolean loop, float volume) {
        ...
        private void applyVolume(Clip clip, float volume) {
            ...
            try {
                if (clip.isControlSupported(FloatControl.Type.MASTER_GAIN)) {
                    ...
                    float dB = (v <= 0f) ? gain.getMinimum()
                            : 20f * (float) Math.log10(v);
    ...
    private static class Version {
        /*
         * 2.5.4.: Lautstärke für Sounds hinzugefügt
         */
    ...
    private class Statistic {
        ...
        private void updateStatistic() {
            ...
            if (timePassed > 1_000) {
                ...
                // PrintObjects
                statisticBox.visibleValue = canvas.printObjects.size();
    ...
    public class TestEnvironment {
        ...
        /**
         * Diese Methode kann einen Tastendruck auf der Tastatur simulieren. Das kann für automatisierte Tests
         * eingesetzt werden. Es wird ein vollständiger Tastendruck simuliert, d.h. es wird ein KeyPressed- und ein
         * KeyReleased-Event erzeugt.
         *
         * @param keyCode Ein KeyCode von <code>java.awt.event.KeyEvent</code>, z.B. <code>KeyEvent.VK_A</code> für den
         *                Buchstaben A.
         */
        ...
        /**
         * Diese Methode kann einen Tastendruck auf der Tastatur simulieren. Das kann für automatisierte Tests
         * eingesetzt werden. Es wird nur ein KeyPressed-Event erzeugt.
         *
         * @param keyCode Ein KeyCode von <code>java.awt.event.KeyEvent</code>, z.B. <code>KeyEvent.VK_A</code> für den
         *                Buchstaben A.
         */
        ...
        /**
         * Diese Methode kann einen Tastendruck auf der Tastatur simulieren. Das kann für automatisierte Tests
         * eingesetzt werden. Es wird nur ein KeyReleased-Event erzeugt.
         *
         * @param keyCode Ein KeyCode von <code>java.awt.event.KeyEvent</code>, z.B. <code>KeyEvent.VK_A</code> für den
         *                Buchstaben A.
         */
